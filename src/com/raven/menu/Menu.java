...
private Icon getMenuIcon(ListModel<String> data, int index) {
    if (data != null && index >= 0 && index < data.getSize()) {
        String element = data.getElementAt(index);

        if (element != null) {
            URL resourceUrl = getClass().getResource("/" + element);

            if (resourceUrl != null) {
                return new ImageIcon(resourceUrl);
            } else {
                // Handle the case where resource is not found
                System.err.println("Resource not found: " + element);
            }
        } else {
            // Handle the case where element is null
            System.err.println("Element at index " + index + " is null");
        }
    } else {
        // Handle the case where data is null or index is out of bounds
        System.err.println("Invalid data or index");
    }
